...
class WebSocketsPool:
    ...
    def create_new_websocket(self):
        self.ws = TwitchWebSocket("wss://pubsub-edge.twitch.tv/v1",
    ...
    def on_message(ws, message):
        ...
        if response["type"] == "MESSAGE":
            ...
            if time.time() - ws.last_message_time < 0.1 and ws.last_message_type == message_type:
            ...
            if topic == "community-points-user-v1":
                if message_type == "points-earned":
                    ...
                    streamer_index = next(i for i, x in enumerate(ws.streamers) if x.channel_id == channel_id)
                    streamer = ws.streamers[streamer_index]

                    new_balance = message_data["balance"]["balance"]
                    logger.info(f"{new_balance} channel points for {streamer}! Reason: {message_data['point_gain']['reason_code']}.")
                elif message_type == "claim-available":
                    channel_id = message_data["claim"]["channel_id"]
                    streamer_index = next(i for i, x in enumerate(ws.streamers) if x.channel_id == channel_id)
                    streamer = ws.streamers[streamer_index]

                    ws.twitch.claim_channel_points_bonus(streamer.username,  message_data["claim"]["id"])

            elif topic == "video-playback-by-id":
                streamer_index = next(i for i, x in enumerate(ws.streamers) if x.channel_id == topic_user)
